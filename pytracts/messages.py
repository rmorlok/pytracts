    ...
    def __new__(cls, name, bases, dct):
        ...
        if bases != (object,):
            ...
            for key, field in dct.items():
                ...
                # Set the name for the field as the variable name, unless it is explicitly named
...
class Message(with_metaclass(_MessageClass, object)):
    ...
    Example:
      ...
      class Order(Message):
        ...
        total_quantity = IntegerField(required=True)
        trade_type = EnumField(TradeType, required=True)
        lots = MessageField(Lot, repeated=True)
    ...
    def get_field_value(self, name):
        """
        Gets the value of the field specified by name, or returns the default value if no value is set.
    ...
    def get_assigned_value(self, name):
        ...
        not return the default for the field. Name can be either a recognized or unrecognized field.

        :param name: Name of attribute to get.

        :returns: Value of attribute, None if it has not been set.

        :raises: KeyError if the name is not set
        """
        if not name:
    ...
    def has_value_assigned(self, name):
        """
        Checks if specified name has a value assigned. Can either be a recognized or unrecognized field.
    ...
    def unset(self, name):
        """
        Mark a field as not set.

        This will make has_value_assigned return False, and getting the value for a known field return the default value.

        :param name: Name of field to reset.
        """
        if name in self.__field_values:
            del self.__field_values[name]
    ...
    def __repr__(self):
        ...
        for field in sorted(self.all_fields(),
            ...
            if self.has_value_assigned(field.name):
                body.append('\n %s: %s' % (attribute, repr(self.get_field_value(field.name))))
...
class Field(with_metaclass(_FieldMeta, object)):
    ...
    def __get__(self, message_instance, message_class):
        ...
        result = message_instance._Message__field_values.get(self.name)
        if result is None:
            return self.default
        else:
            return result
...
class MessageField(Field):
    ...
    def __init__(self,
        ...
        super(MessageField, self).__init__(name=name,
                                           required=required,
                                           repeated=repeated,
                                           variant=variant)
